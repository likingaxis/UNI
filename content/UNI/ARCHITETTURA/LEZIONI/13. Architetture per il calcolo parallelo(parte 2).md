...
>quando per eseguire una attività hai bisogno di più risorse di quante ne dovresti necessitare
...
### 1. Soft**Le primitive send e receive possono essere bloccanti (sincrone) o non bloccanti (asincrone).**ware di comunicazione di basso livello
...
Il modello a scambio di messaggi è molto conveniente per un sistema operativo multi computer ma soffre di un grave difetto ovvero tutti le comunicazioni (i programmi) utilizzano l'I/O. È possibile bypassare ciò consentendo ai programmi di chiamare direttamente le procedure che si trovano su altre CPU