...
hanno una memoria condivisa e grazie a questo non serve installare un sistema operativo per ogni CPU, inoltre può esserci una sola CPU che si occupa degli scambi I/O(multiprocessore asimmetrico) esistono due altri tipi di multiprocessori UMA(quando le parole vengono lette in memoria sempre alla stessa velocità Uniform Memory Access) oppure NUMA(Non Uniform Memory Access)
...
Per n CPU e n memorie $\dfrac{n}{2}\log_2(x)$
